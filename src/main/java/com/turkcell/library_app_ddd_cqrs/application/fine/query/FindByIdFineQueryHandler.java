...
public class FindByIdFineQueryHandler implements QueryHandler<FindByIdFineQuery, FineResponse> {
    private final FineRepository fineRepository;
    private final FineResponseMapper fineResponseMapper;

    @Override
    public FineResponse handle(FindByIdFineQuery query) {
        return fineResponseMapper.toResponse(fineRepository.findById(new FineId(query.id())).get());
    }
